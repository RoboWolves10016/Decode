...
//        follower = Constants.createFollower(hardwareMap);
        follower.setStartingPose(BlueGoalPaths.startPose);
...
//                follower.followPath(path);
    ...
    private void createPaths() {
        startToLaunch = follower
                .pathBuilder()
                .addPath(
                        new BezierLine(BlueGoalPaths.startPose, BlueGoalPaths.launchPose)
                )
                .setConstantHeadingInterpolation(Math.toRadians(180-35.3))
                .build();
